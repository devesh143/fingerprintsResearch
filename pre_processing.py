...
def makeTrainImages(img_path, finger_id, j, save_url = 'E:/contact-contactless-fingerprint-contact/dataR'):
    finger_idx = 1
...
contact_images = 'E:/contact-contactless-fingerprint-contact/contact-based_fingerprints'
from os.path import join
num_augmented_images = 1000

for finger_id in range(13, 336 + 1):
    print(f"[*] creating {finger_id}...")
    all_images = glob(join(contact_images, '**/*.jpg'))
    images_to_process = all_images[:num_augmented_images]
    for j, img_path in enumerate(images_to_process):
        makeTrainImages(img_path, finger_id, j + 1, 'E:/contact-contactless-fingerprint-contact/dataR')
